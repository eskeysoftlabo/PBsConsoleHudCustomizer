local strings = {
	...
	SI_PBSCHC_TIMER_MODE_TOOLTIP = "The other set always gets this add-on's countdown, because the game never draws one there. On the set you are on the game may be drawing its own, under Settings > Interface > Action Bar Timers: Automatic leaves that one alone and only draws when it is off, Always draws ours as well, Never leaves the bar to the game.",
	SI_PBSCHC_TIMER_MODE_AUTO = "Automatic",
	SI_PBSCHC_TIMER_MODE_ALWAYS = "Always",
	SI_PBSCHC_TIMER_MODE_NEVER = "Never",
	...
	SI_PBSCHC_COUNT_FROM_ONE_TOOLTIP = "Write the count even when there is only one target. Off, the number appears from two targets, so a single-target ability does not carry a 1 for its whole duration.",
