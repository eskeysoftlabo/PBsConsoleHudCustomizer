local strings = {
	...
	SI_PBSCHC_STYLE_TOOLTIP = "「標準」はゲーム本来のバーのままです。「リキッド」は標準の形・枠・光沢を保ち、半透明の塗りつぶしに光の筋が流れ、小さな泡が浮き上がります。「四角」は各バーを平坦な長方形で描きます（暗いトラックと、そのリソース本来の色で塗られた四角）。大きさはゲーム本来のままで、倍率で調整します。「MURA-HIGE Style」は同じ四角を、下で指定した幅と高さで描きます。「MURA-HIGE NEO Style」は同じ幅・高さの設定で、体力・マジカ・スタミナすべてが左から右へ満ちます。これらのスタイルはゲーム側の矢印型の枠と背景を一時的に非表示にしますが、バー自体はそのまま残して動かしているので、ダメージシールドや防御力変化、瀕死の警告表示は今までどおり上に表示されます。",
