local strings = {
	...
	SI_PBSCHC_TIMER_MODE_TOOLTIP = "裏バーには常にこのアドオンの残り時間を表示します（ゲーム側は裏バーに数字を出さないため）。表バーについては、ゲーム側の「設定 > インターフェース > アクションバータイマー」がオンだと数字が二重になるため、「自動」ではそれがオフのときだけ表示します。「常に表示」はゲーム側と併せて表示、「表示しない」は表バーをゲームに任せます。",
	SI_PBSCHC_TIMER_MODE_AUTO = "自動",
	SI_PBSCHC_TIMER_MODE_ALWAYS = "常に表示",
	SI_PBSCHC_TIMER_MODE_NEVER = "表示しない",
	...
	SI_PBSCHC_COUNT_FROM_ONE_TOOLTIP = "対象が1体のときにも数字を表示します。オフの場合は2体以上のときだけ表示するため、単体スキルに常に1が出ることがありません。",
