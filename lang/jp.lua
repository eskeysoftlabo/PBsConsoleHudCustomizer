local strings = {
	...
	SI_PBSCHC_STYLE_TOOLTIP = "「標準」はゲーム本来のバーのままです。「リキッド」はゲーム本来の矢印型の枠の代わりに四角い枠を使い、中身を四角いガラス管に入った半透明の液体のように見せます。柔らかな光の揺らぎが流れ、量が変わると液面が波打ち、減った分は流れ落ちるように消え、泡が浮き上がり、ガラスに光が走ります。「四角」は各バーを平坦な長方形で描きます（暗いトラックと、そのリソース本来の色で塗られた四角）。大きさはゲーム本来のままで、倍率で調整します。「MURA-HIGE Style」は同じ四角を、下で指定した幅と高さで描きます。「MURA-HIGE NEO Style」は同じ幅・高さの設定で、体力・マジカ・スタミナすべてが左から右へ満ちます。これらのスタイルはゲーム側の矢印型の枠と背景を一時的に非表示にしますが、バー自体はそのまま残して動かしているので、ダメージシールドや防御力変化、瀕死の警告表示は今までどおり上に表示されます。",
	...
	SI_PBSCHC_BORDER_COLOUR_TOOLTIP = "四角・MURA-HIGE Style・MURA-HIGE NEO Style・リキッドの枠線の色です。体力・マジカ・スタミナに共通で適用します。「枠線を描く」がオンのときに表示されます。",
	...
	SI_PBSCHC_PLAIN_BORDER_TOOLTIP = "各バーの周囲に選択した色の細い線を描きます（四角・MURA-HIGE系・リキッド）。ゲーム本来の矢印型の枠は、これらのスタイルを選んでいる間は常に非表示になるため（平坦な四角を矢印型の枠に入れても中途半端なため）、枠としてはこれだけになります。オフにすると、線のない色の塊だけになります。",
